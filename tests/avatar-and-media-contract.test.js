...
const functions = read('../functions/index.js');
  ...
  const supabaseMedia = read('../src/services/supabaseMediaService.js');
...
const chatPage = read('../src/pages/ChatPage.jsx');
  ...
  const publicProfile = read('../src/components/profile/PublicProfileView.jsx');
...
const publicChatPath = 'chat_media/rooms/uid123/principal/message123/asset123.jpg';
 ...
 describe('contratos locales de avatar y fotos', () => {
  ...
  it('mantiene el upload público Supabase y conserva reglas históricas aisladas', () => {
    expect(chatInput).toContain('uploadPublicChatPhotoToSupabase');
    expect(chatInput).not.toContain('chat_media/rooms/${user?.id || \'unknown\'}/${roomId || \'principal\'}/${tempMessageId}/${assetId}.${extension}');
  ...
  it('exige metadatos de imagen coherentes con la subida del cliente', () => {
    ...
    expect(chatInput).toContain('mimeType: uploaded.mimeType');
    expect(chatInput).toContain('size: uploaded.size');
  ...
  it('importa el compresor usado por las subidas Supabase', () => {
    expect(supabaseMedia).toContain("import imageCompression from 'browser-image-compression';");
    expect(supabaseMedia).toContain('compressSupabaseImage');
  });
  ...
  it('mantiene protegido el upload de perfil y no fabrica IDs de invitado', () => {
    ...
    expect(profileService).toContain('SUPABASE_REQUIRED_FOR_PROFILE_PHOTOS');
    expect(profileService).not.toContain('api.cloudinary.com');
    expect(chatInput).toContain('uploadPublicChatPhotoToSupabase');
    expect(chatInput).toContain('SUPABASE_REQUIRED_FOR_PUBLIC_MEDIA');
    expect(chatInput).toContain("roomId === 'principal' && isSupabaseAuthEnabled()");
  });

  it('mantiene el selector de foto de perfil con control semántico y teclado nativo', () => {
    const profileModal = read('../src/components/profile/PhotoUploadModal.jsx');
    expect(profileModal).toContain('<label');
    expect(profileModal).toContain('htmlFor="profile-photo-input"');
    expect(profileModal).toContain('id="profile-photo-input"');
    expect(profileModal).not.toContain('role="button"');
    expect(profileModal).toContain('SUPABASE_REQUIRED_FOR_PROFILE_PHOTOS');
    expect(profileModal).toContain('const profileUpdated = await updateProfile({ avatar: photoURL });');
    expect(profileModal).toContain("if (!profileUpdated)");
  });

  it('normaliza avatares en perfiles públicos y no muestra cero cuando falta la métrica', () => {
    expect(publicProfile).toContain('getSafeAvatarSrc(profile.avatar)');
    expect(publicProfile).toContain('getSafeAvatarSrc(friend.avatar)');
    expect(publicProfile).toContain('onError={handleAvatarImageError}');
    expect(publicProfile).toContain("if (value === null || value === undefined || value === '') return '—';");
    expect(publicProfile).toContain('Participación verificada');
  });
  ...
  it('usa el helper global en ChatPage y reporta imágenes compartidas rotas', () => {
    ...
    expect(chatPage).toContain('return Promise.all([validationPromise])');
    expect(chatPage).toContain("throw new Error('MESSAGE_NOT_CONFIRMED')");
...
describe('contratos locales del chat privado activo', () => {
  it('usa identidad pública segura, typing reglado y media privada restringida', () => {
    ...
    expect(privateChat).toContain('uploadPrivateChatPhotoToSupabase');
    expect(privateChat).toContain('SUPABASE_REQUIRED_FOR_PRIVATE_MEDIA');
    expect(privateChat).not.toContain('chat_media/private/${user?.id || \'unknown\'}/${chatId}/${tempMessageId}/${assetId}.${extension}');
    ...
    expect(socialService).toContain('SUPABASE_REQUIRED_FOR_PRIVATE_MEDIA');
